...
void main()
{
    vec3 nor = getNor(texture2DLod(gbuffer0, texCoord, 0.0).xy);
    gl_FragData[0] = texture2DLod(tex, texCoord, 0.0).x * 0.132571995258331298828125;
    float weight = 0.132571995258331298828125;
    for (int i = 1; i < 8; i++)
    {
        float posadd = float(i);
        vec3 nor2 = getNor(texture2DLod(gbuffer0, texCoord + (dirInv * float(i)), 0.0).xy);
        float influenceFactor = step(0.949999988079071044921875, dot(nor2, nor));
        float col = texture2DLod(tex, texCoord + (dirInv * posadd), 0.0).x;
        ...
        gl_FragData[0] += (col * w);
        weight += w;
        nor2 = getNor(texture2DLod(gbuffer0, texCoord - (dirInv * float(i)), 0.0).xy);
        influenceFactor = step(0.949999988079071044921875, dot(nor2, nor));
        col = texture2DLod(tex, texCoord - (dirInv * posadd), 0.0).x;
