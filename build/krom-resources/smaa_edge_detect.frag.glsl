...
vec2 SMAAColorEdgeDetectionPS(vec2 texcoord)
{
    ...
    vec3 C = texture2DLod(colorTex, texcoord, 0.0).xyz;
    vec3 Cleft = texture2DLod(colorTex, offset0.xy, 0.0).xyz;
    vec3 t = abs(C - Cleft);
    vec4 delta;
    delta.x = max(max(t.x, t.y), t.z);
    vec3 Ctop = texture2DLod(colorTex, offset0.zw, 0.0).xyz;
    t = abs(C - Ctop);
    ...
    vec3 Cright = texture2DLod(colorTex, offset1.xy, 0.0).xyz;
    t = abs(C - Cright);
    delta.z = max(max(t.x, t.y), t.z);
    vec3 Cbottom = texture2DLod(colorTex, offset1.zw, 0.0).xyz;
    t = abs(C - Cbottom);
    delta.w = max(max(t.x, t.y), t.z);
    vec2 maxDelta = max(delta.xy, delta.zw);
    vec3 Cleftleft = texture2DLod(colorTex, offset2.xy, 0.0).xyz;
    t = abs(C - Cleftleft);
    delta.z = max(max(t.x, t.y), t.z);
    vec3 Ctoptop = texture2DLod(colorTex, offset2.zw, 0.0).xyz;
    t = abs(C - Ctoptop);
    delta.w = max(max(t.x, t.y), t.z);
