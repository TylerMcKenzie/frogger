...
{
    vec3 _239 = vec3(uv + (vec2(-1.0) / smSize), compare);
    float result = texture2D(shadowMap_1, vec3(_239.xy, _239.z));
    vec3 _248 = vec3(uv + (vec2(-1.0, 0.0) / smSize), compare);
    result += texture2D(shadowMap_1, vec3(_248.xy, _248.z));
    vec3 _259 = vec3(uv + (vec2(-1.0, 1.0) / smSize), compare);
    result += texture2D(shadowMap_1, vec3(_259.xy, _259.z));
    vec3 _270 = vec3(uv + (vec2(0.0, -1.0) / smSize), compare);
    result += texture2D(shadowMap_1, vec3(_270.xy, _270.z));
    vec3 _278 = vec3(uv, compare);
    ...
    vec3 _300 = vec3(uv + (vec2(1.0, -1.0) / smSize), compare);
    result += texture2D(shadowMap_1, vec3(_300.xy, _300.z));
    vec3 _311 = vec3(uv + (vec2(1.0, 0.0) / smSize), compare);
    result += texture2D(shadowMap_1, vec3(_311.xy, _311.z));
    vec3 _322 = vec3(uv + (vec2(1.0) / smSize), compare);
...
float shadowTestCascade(sampler2DShadow shadowMap_1, vec3 eye_1, vec3 p, float shadowsBias_1)
{
    ...
    int param;
    int param_1;
    mat4 _418 = getCascadeMat(d, param, param_1);
    int casi = param;
    int casIndex = param_1;
    mat4 LWVP = _418;
    vec4 lPos = LWVP * vec4(p, 1.0);
    vec3 _433 = lPos.xyz / vec3(lPos.w);
    ...
    if ((splitDist <= 0.1500000059604644775390625) && (casi != 3))
    {
        ...
        vec3 _537 = lPos2.xyz / vec3(lPos2.w);
...
void main()
{
    vec4 g0 = texture2DLod(gbuffer0, texCoord, 0.0);
    vec3 n;
    ...
    vec3 _849 = gl_FragData[0].xyz * texture2DLod(ssaotex, texCoord, 0.0).x;
    ...
    vec3 _906 = gl_FragData[0].xyz + ((sdirect * svisibility) * sunCol);
